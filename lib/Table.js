...
Table.prototype.handle = function (infos) {
  var self = this;
  var match, reg;

  // Read captions and id
  if(infos.caption) {
    match = this._handleCaption(infos.caption)
  ...
  // Read headers
  if(infos.headers) {
    this.headers = this._handleHeaders(infos.headers);
  }

  // Read columns
  this.colgroup.parse(infos.columns)

  // Read rows
  if(infos.rows) {
    infos.rows.trim().split(/\n[\s\.]*\n/g).forEach(function(body) {
      var tbody = new TBody()
      tbody.parse(body)
