...
var wholeTable   =
  ...
  + '('+ firstRow				    // First line must start at beginning
  + '('+ tableRow +')*?)?'	 // Header Rows
  + tableDivider			       // Divider/Alignment definitions
  + tableRows+'+'			      // Body Rows
  + '('+ tableCaption +')?'	// Optional caption (TODO: Not used)
...
Document._parse = function (src) {
  var reTable = new RegExp('^(' + wholeTable + ')(\n|\\Z)','mg')
  var result, tables = [];
...
Table.prototype._parse = function () {
  ...
  raw = raw.replace(reCaption, function(match, c1, c2) {
    self.id = slugify(c2);
